...
const toolName = 'tools/world-scene-menu-entity-routine-confidence-backfill-audit.mjs';
const catalogId = 'world-scene-menu-entity-routine-confidence-backfill-catalog-2026-06-26';
const reportId = 'scene-menu-entity-routine-confidence-backfill-audit-2026-06-26';
const schemaVersion = 1;

const evidenceFamilies = {
  scene_transition_runtime: new Set([
    'bank2HudCounterAudit',
    'bank2MotionSequenceAudit',
    'bank2SceneMotionAudit',
    'bank2SceneRoutineAudit',
    'bank2TransitionRoutineAudit',
    'bank2VdpRootProducerAudit',
    'bank2VdpStreamStateAudit',
    'd1aeTransitionControllerBridgeAudit',
    'd1afSceneCompletionAudit',
  ]),
  menu_status_ui_runtime: new Set([
    'bank0ContinueTransitionAudit',
    'bank0MenuRoutineAudit',
    'bank0StatusInventoryAudit',
    'cf52StatusScrollAdjustAudit',
    'cf5bFormStageProgressionAudit',
    'cf6aDispatchProducerAudit',
    'cf6aRequest3FormTransitionAudit',
    'spritePaletteWriterAudit',
    'statusVdpWriterDetailAudit',
    'uiPlayerTransitionTableAudit',
    'uiTriggerRoutineAudit',
  ]),
  core_vdp_runtime: new Set([
    'bank0CoreHelperAudit',
    'bank0CoreRuntimeAudit',
    'bank0LowcoreVdpAudit',
    'vdpRenderRoutineAudit',
  ]),
  entity_aux_runtime: new Set([
    'auxEntityRoutineAudit',
    'bank0EntityBehaviorAudit',
    'bank0EntityInitHeadsAudit',
    'c3c0ActorFamilyAudit',
    'c3c0BehaviorTargetSemanticsAudit',
    'c3c0FrameStepControlFlowAudit',
    'c3c0FrameStepControlFlowTargetAudit',
    'c3c0FrameStepDiagnosticAudit',
    'c3c0FrameStepDiagnosticTargetAudit',
    'c3c0FrameStepTraceAudit',
    'c3c0FrameStepTraceTargetAudit',
    'c3c0MotionSeedFamilyAudit',
    'c3c0MotionSeedTargetLinkAudit',
    'c3c0RenderabilityAudit',
    ...
    'entityMotionCollisionHelperAudit',
...
function wasGeneratedByThisAudit(region) {
  return region.analysis?.sceneMenuEntityRoutineConfidenceBackfillAudit?.catalogId === catalogId;
}
...
function buildCatalog(mapData) {
  const entries = (mapData.regions || [])
    ...
    .map(region => {
      ...
      const existingAudit = region.analysis?.sceneMenuEntityRoutineConfidenceBackfillAudit || null;
      return {
  ...
  return {
    ...
    sourceCatalogs: [
      ...
      'world-bank0-menu-routine-catalog-2026-06-25',
      'world-ui-trigger-routine-catalog-2026-06-25',
      'world-vdp-render-routine-catalog-2026-06-25',
      'world-entity-runtime-routine-catalog-2026-06-25',
      'world-bank0-entity-behavior-catalog-2026-06-25',
    ],
    assetPolicy: 'Metadata only: routine ids, offsets, sizes, subsystem families, evidence audit keys, catalog ids, and aggregate counts. No ROM bytes, instruction bytes, decoded assets, graphics, screenshots, pixels, audio payloads, or hashes are embedded.',
    selectionRule: {
      includedType: 'code',
      evidenceFamilies: Object.fromEntries(Object.entries(evidenceFamilies).map(([family, keys]) => [family, [...keys].sort()])),
      evidenceRule: 'A code region must already have high or medium_high evidence from one of the selected scene/transition, menu/status/UI, core VDP/runtime, or entity/auxiliary runtime audits. Generic ASM labels, inferred metadata, and callsite-only evidence do not qualify.',
...
function annotateMap(mapData, catalog) {
  ...
  for (const region of mapData.regions || []) {
    ...
    if (apply) {
      ...
      region.analysis.sceneMenuEntityRoutineConfidenceBackfillAudit = {
        catalogId,
        kind: 'scene_menu_entity_runtime_routine_top_level_confidence_backfill',
        ...
        summary: 'Backfilled top-level confidence for scene, menu/status/UI, core VDP/runtime, or entity/auxiliary code routines from existing subsystem evidence.',
        generatedAt: now,
        tool: toolName,
      };
    }
...
function main() {
  ...
  if (apply) {
    ...
    mapData.analysisReports.push({
      id: reportId,
      type: 'scene_menu_entity_routine_confidence_backfill_audit',
      generatedAt: now,
      tool: `${toolName} --apply`,
      schemaVersion,
      catalogId,
      ...
      changedRegions,
      evidence: [
        'Selected code regions already carry high or medium_high subsystem evidence in map.json.',
        'Generic ASM label/incbin evidence, inferred metadata, and callsite-only evidence are excluded.',
        'This audit promotes routine metadata confidence; it does not claim JavaScript engine behavior reconstruction is complete.',
        'No ROM bytes, instruction bytes, decoded assets, graphics, screenshots, pixels, audio payloads, or hashes are persisted.',
      ],
      nextLeads: [
        'Split the promoted scene/menu/entity routines into RAM/effect catalogs before converting behavior into JavaScript modules.',
        'Use family counts to prioritize separate frame-by-frame traces for scene transitions, menu/status rendering, and entity behavior.',
        'Keep remaining unpromoted code regions pending until they have subsystem-specific evidence stronger than generic labels or callsites.',
      ],
    });
